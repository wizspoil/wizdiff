...
class WizDiffDatabase:
    ...
    async def close(self):
        if self._db_connection:
            discarded_transaction = self._db_connection.in_transaction
            await self._db_connection.close()
            self._db_connection = None
            if discarded_transaction:
                raise RuntimeError("Closed database with an uncommitted transaction.")

    async def init_database(self):
        connection = await self._connection()
        async with connection.executescript(TABLE_SCRIPT):
            await self.commit()
